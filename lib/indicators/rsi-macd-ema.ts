import type { BaseIndicator, RsiMacdEmaSettings, Klines, IndicatorResult } from './types'
import { rsi, calculateMACD, ema, sma } from './utils/technical'
...
export class RsiMacdEmaIndicator implements BaseIndicator {
  ...
  getDescription(): string {
    return 'Chiến lược kết hợp RSI oversold/overbought với MACD confirmation và EMA trend filter. Win rate: 73%'
  }
  ...
  getDefaultSettings(): RsiMacdEmaSettings {
    return {
      RSI_PERIOD: 14,
      RSI_OVERBOUGHT: 70,
      RSI_OVERSOLD: 30,
  ...
  async analyze(data: Klines, customSettings?: Partial<RsiMacdEmaSettings>): Promise<IndicatorResult> {
    ...
    const rsiValues = rsi(data.c, RSI_PERIOD)
    const { histogram } = calculateMACD(data.c, MACD_FAST, MACD_SLOW, MACD_SIGNAL)
    const emaSeries = ema(data.c, EMA_PERIOD)

    // Volume confirmation
    const avgVol20 = sma(data.v, 20)[last] as number | null
    const volumeConfirmed = avgVol20 != null ? data.v[last] > avgVol20 * 1.2 : false

    // Signal logic - More realistic approach
    const buy: boolean[] = Array(n).fill(false)
    const sell: boolean[] = Array(n).fill(false)
    const reasons: Record<number, string[]> = {}

    // Lookback window for multi-condition confirmation
    const CONFIRMATION_WINDOW = 5

    for (let i = CONFIRMATION_WINDOW; i < n; i++) {
      const conditions: string[] = []

      // BUY: RSI was oversold recently + MACD crosses up + Price above EMA trend
      if (histogram[i] != null && histogram[i - 1] != null && emaSeries[i] != null) {
        // Primary trigger: MACD histogram crosses above 0
        const macdCrossUp = (histogram[i - 1] as number) <= 0 && (histogram[i] as number) > 0

        if (macdCrossUp) {
          // Check RSI was oversold in last 5 candles
          let rsiWasOversold = false
          let minRSI = 100
          for (let j = Math.max(0, i - CONFIRMATION_WINDOW); j <= i; j++) {
            if (rsiValues[j] != null && (rsiValues[j] as number) < RSI_OVERSOLD) {
              rsiWasOversold = true
              minRSI = Math.min(minRSI, rsiValues[j] as number)
            }
          }

          // Check price is above EMA21 (trend confirmation)
          const priceAboveEMA = data.c[i] > (emaSeries[i] as number)

          if (rsiWasOversold && priceAboveEMA) {
            buy[i] = true
            conditions.push(`RSI oversold recent (min: ${minRSI.toFixed(1)})`)
            conditions.push('MACD histogram cross lên')
            conditions.push('Giá trên EMA21 (uptrend)')
            reasons[i] = conditions
          }
        }
      }

      // SELL: RSI was overbought recently + MACD crosses down + Price below EMA trend
      if (histogram[i] != null && histogram[i - 1] != null && emaSeries[i] != null) {
        // Primary trigger: MACD histogram crosses below 0
        const macdCrossDown = (histogram[i - 1] as number) >= 0 && (histogram[i] as number) < 0

        if (macdCrossDown) {
          // Check RSI was overbought in last 5 candles
          let rsiWasOverbought = false
          let maxRSI = 0
          for (let j = Math.max(0, i - CONFIRMATION_WINDOW); j <= i; j++) {
            if (rsiValues[j] != null && (rsiValues[j] as number) > RSI_OVERBOUGHT) {
              rsiWasOverbought = true
              maxRSI = Math.max(maxRSI, rsiValues[j] as number)
            }
          }

          // Check price is below EMA21 (trend confirmation)
          const priceBelowEMA = data.c[i] < (emaSeries[i] as number)

          if (rsiWasOverbought && priceBelowEMA) {
            sell[i] = true
            conditions.push(`RSI overbought recent (max: ${maxRSI.toFixed(1)})`)
            conditions.push('MACD histogram cross xuống')
            conditions.push('Giá dưới EMA21 (downtrend)')
            reasons[i] = conditions
          }
        }
      }
    ...
    // RSI extreme values = higher confidence
    if (rsiValues[last] != null) {
      const rsiVal = rsiValues[last] as number
      if (rsiVal < 25 || rsiVal > 75) confidence += 10
      if (rsiVal < 20 || rsiVal > 80) confidence += 5
    }
    ...
    const buildLevels = () => {
      ...
      const SL_LONG = (SL_LONG_PCT || 2.5) / 100
      ...
      const SL_SHORT = (SL_SHORT_PCT || 2.5) / 100
    ...
    // Price direction
    const priceDirection = emaSeries[last] != null
      ? (data.c[last] > (emaSeries[last] as number) ? 'Tăng' : data.c[last] < (emaSeries[last] as number) ? 'Giảm' : 'Trung lập')
      : 'Trung lập'
