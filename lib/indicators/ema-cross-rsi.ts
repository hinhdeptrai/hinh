import type { BaseIndicator, EmaCrossRsiSettings, Klines, IndicatorResult } from './types'
import { ema, calculateRSI, sma } from './utils/technical'

export class EmaCrossRsiIndicator implements BaseIndicator {
  getName(): string {
    return 'EMA Cross + RSI Confirmation'
  }

  getDescription(): string {
    return 'Chiến lược EMA crossover với RSI confirmation. Win rate: 60%'
  }
  ...
  getDefaultSettings(): EmaCrossRsiSettings {
    return {
      EMA_FAST: 12,
      EMA_SLOW: 26,
      RSI_PERIOD: 14,
      RSI_CONFIRMATION: 50, // RSI > 50 for buy, RSI < 50 for sell
  ...
  async analyze(data: Klines, customSettings?: Partial<EmaCrossRsiSettings>): Promise<IndicatorResult> {
    ...
    for (let i = Math.max(EMA_SLOW, RSI_PERIOD) + 1; i < n; i++) {
      const conditions: string[] = []

      // BUY: Fast EMA crosses above Slow EMA + RSI > confirmation level
      if (
        emaFast[i] != null &&
        emaSlow[i] != null &&
        emaFast[i - 1] != null &&
        ...
        (rsi[i] as number) > RSI_CONFIRMATION // RSI confirmation
      ) {
        buy[i] = true
        conditions.push(`EMA ${EMA_FAST} cross lên trên EMA ${EMA_SLOW}`)
        conditions.push(`RSI ${(rsi[i] as number).toFixed(1)} > ${RSI_CONFIRMATION}`)
        reasons[i] = conditions
      }

      // SELL: Fast EMA crosses below Slow EMA + RSI < confirmation level
      if (
        emaFast[i] != null &&
        emaSlow[i] != null &&
        emaFast[i - 1] != null &&
    ...
    // Add RSI strength factor
    if (rsi[last] != null) {
      const rsiVal = rsi[last] as number
      // Strong RSI confirmation
      if (emaFast[last] != null && emaSlow[last] != null) {
        if ((emaFast[last] as number) > (emaSlow[last] as number) && rsiVal > 60) {
          confidence += 15 // Strong bullish
        } else if ((emaFast[last] as number) < (emaSlow[last] as number) && rsiVal < 40) {
          confidence += 15 // Strong bearish
        }
      }
    }
    // Add EMA divergence factor
    if (emaFast[last] != null && emaSlow[last] != null) {
      const emaDiff = Math.abs(((emaFast[last] as number) - (emaSlow[last] as number)) / (emaSlow[last] as number))
      if (emaDiff > 0.02) confidence += 10 // Strong EMA divergence
    }
    ...
    // Price direction based on EMA
    const priceDirection = emaFast[last] != null && emaSlow[last] != null
      ? ((emaFast[last] as number) > (emaSlow[last] as number) ? 'Tăng' : (emaFast[last] as number) < (emaSlow[last] as number) ? 'Giảm' : 'Trung lập')
      : 'Trung lập'
