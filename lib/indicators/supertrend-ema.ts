import type { BaseIndicator, SupertrendEmaSettings, Klines, IndicatorResult } from './types'
import { calculateSupertrend, ema, sma } from './utils/technical'

export class SupertrendEmaIndicator implements BaseIndicator {
  getName(): string {
    return 'Supertrend + EMA Cross'
  }

  getDescription(): string {
    return 'Chiến lược kết hợp Supertrend direction với EMA crossover. Win rate: 65%'
  }
  ...
  getDefaultSettings(): SupertrendEmaSettings {
    return {
  ...
  async analyze(data: Klines, customSettings?: Partial<SupertrendEmaSettings>): Promise<IndicatorResult> {
    ...
    const { supertrend, direction } = calculateSupertrend(data.h, data.l, data.c, SUPERTREND_PERIOD, SUPERTREND_MULTIPLIER)
    const emaFast = ema(data.c, EMA_FAST)
    const emaSlow = ema(data.c, EMA_SLOW)

    // Volume confirmation
    const avgVol20 = sma(data.v, 20)[last] as number | null
    const volumeConfirmed = avgVol20 != null ? data.v[last] > avgVol20 * 1.2 : false
    ...
    for (let i = Math.max(EMA_SLOW, SUPERTREND_PERIOD) + 1; i < n; i++) {
      const conditions: string[] = []

      // BUY: Supertrend turns bullish + Fast EMA crosses above Slow EMA
      if (
        direction[i] === 1 &&
        direction[i - 1] === -1 && // Supertrend just turned bullish
        emaFast[i] != null &&
        emaSlow[i] != null &&
        (emaFast[i] as number) > (emaSlow[i] as number) // EMA bullish alignment
      ) {
        buy[i] = true
        conditions.push('Supertrend chuyển sang bullish')
        conditions.push(`EMA ${EMA_FAST} > EMA ${EMA_SLOW}`)
        reasons[i] = conditions
      }

      // Alternative BUY: Supertrend already bullish + Fast EMA just crossed above Slow EMA
      if (
        direction[i] === 1 &&
        emaFast[i] != null &&
        emaSlow[i] != null &&
        emaFast[i - 1] != null &&
        emaSlow[i - 1] != null &&
        (emaFast[i - 1] as number) <= (emaSlow[i - 1] as number) &&
        (emaFast[i] as number) > (emaSlow[i] as number) // EMA cross up
      ) {
        if (!buy[i]) {
          buy[i] = true
          conditions.push('Supertrend đang bullish')
          conditions.push(`EMA ${EMA_FAST} cross lên trên EMA ${EMA_SLOW}`)
          reasons[i] = conditions
        }
      }

      // SELL: Supertrend turns bearish + Fast EMA crosses below Slow EMA
      if (
        direction[i] === -1 &&
        direction[i - 1] === 1 && // Supertrend just turned bearish
        emaFast[i] != null &&
        emaSlow[i] != null &&
        (emaFast[i] as number) < (emaSlow[i] as number) // EMA bearish alignment
      ) {
        sell[i] = true
        conditions.push('Supertrend chuyển sang bearish')
        conditions.push(`EMA ${EMA_FAST} < EMA ${EMA_SLOW}`)
        reasons[i] = conditions
      }

      // Alternative SELL: Supertrend already bearish + Fast EMA just crossed below Slow EMA
      if (
        direction[i] === -1 &&
        emaFast[i] != null &&
        emaSlow[i] != null &&
        ...
        emaSlow[i - 1] != null &&
        (emaFast[i - 1] as number) >= (emaSlow[i - 1] as number) &&
        (emaFast[i] as number) < (emaSlow[i] as number) // EMA cross down
      ) {
        if (!sell[i]) {
          sell[i] = true
          conditions.push('Supertrend đang bearish')
          conditions.push(`EMA ${EMA_FAST} cross xuống dưới EMA ${EMA_SLOW}`)
          reasons[i] = conditions
        }
      }
    }
    ...
    if (direction[last] === 1 && emaFast[last] != null && emaSlow[last] != null) {
      const emaDiff = Math.abs(((emaFast[last] as number) - (emaSlow[last] as number)) / (emaSlow[last] as number))
      if (emaDiff > 0.02) confidence += 15 // Strong EMA divergence
    } else if (direction[last] === -1 && emaFast[last] != null && emaSlow[last] != null) {
      const emaDiff = Math.abs(((emaFast[last] as number) - (emaSlow[last] as number)) / (emaSlow[last] as number))
      if (emaDiff > 0.02) confidence += 15 // Strong EMA divergence
    }
    ...
    // Price direction based on Supertrend
    const priceDirection = direction[last] === 1 ? 'Tăng' : direction[last] === -1 ? 'Giảm' : 'Trung lập'
